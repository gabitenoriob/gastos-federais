...
def carregar_csv(caminho_csv):
    ...
    df["valor_liquidado"] = df["valor_liquidado"].astype(str).str.replace(".", "", regex=True).str.replace(",", ".", regex=True)
    df["valor_liquidado"] = pd.to_numeric(df["valor_liquidado"], errors='coerce').fillna(0)

    df["valor_empenhado"] = df["valor_empenhado"].astype(str).str.replace(".", "", regex=True).str.replace(",", ".", regex=True)
    df["valor_empenhado"] = pd.to_numeric(df["valor_empenhado"], errors='coerce').fillna(0)

    df["valor_pago"] = df["valor_pago"].astype(str).str.replace(".", "", regex=True).str.replace(",", ".", regex=True)
...
def inserir_dados_sql(df):
    ...
    for index, row in df.iterrows():
        valores = tuple(row.fillna("").to_dict().values())  # Substitui NaN por string vazia e transforma em tupla

        placeholders = ", ".join(["?"] * len(row))  # Cria placeholders dinâmicos (?, ?, ?, ...)
...
caminho_arquivo ="dados\\recursos recebidos al\\dados.csv"
